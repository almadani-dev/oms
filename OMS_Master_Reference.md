...
- **Audited subjects (11, and only these):** `project`, `project_cost`, `partner`, `partner_type`, `project_super`, `project_status`, `bank_type`, `fiscal_year`, `transaction_type`, `transaction_super_type`, `setting`. Every event uses `event_category = crud` and one of `created`/`updated`/`deleted`/`restored`. **Nothing financial is audited yet** — `Transaction`, `TransactionLine`, `ProjectCostReceipt`, `ProjectCostBudget`, `ProjectCostBudgetsPayment`, `GeneralExpense`, `GeneralExchange`, `Account`, `AccountType`, `Currency`, `ExchangeRateHistory` — and neither are `User`/`Role`/`Permission`, authentication, attachments, report exports, or backup/restore. `Account`/`AccountType` are still **not** wired to `HasUserTracking`.

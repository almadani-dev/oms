...
- `accounts.account_code` is nullable by design (original migration + `AccountForm` + every read site's `$account->account_code ? ... : ''` guard all agree); a live-only `NOT NULL` drift was reconciled to match in OMS Task 8.2, 2026-07-28. **It is also NO LONGER UNIQUE as of 2026-08-16** (migration `2026_08_16_100000`): the UNIQUE index was replaced with a plain `accounts_account_code_index`, because `account_code` is the real bank/wallet number a human types and two beneficiary accounts may legitimately share one. Accounts stay financially distinct because every `transaction_line` references `accounts.id`. A repo-wide audit first confirmed no production code treats the code as an identifier — all six usages are `"code - name"` label building or `orderBy`, and the single `where('account_code', …)` is `CreateAccount::resolveOpeningClearingAccount()`'s probe for a free generated `OPB-{CURRENCY}` code. **The rollback is not restorable once duplicates exist and fails loudly by design** — see `docs/DECISIONS_LOG.md` (2026-08-16).
...
**Muwakha families (3 tables — OMS Muwakha Families, 2026-08-16; account model finalized 2026-08-17):**
- `muwakha_families` — the beneficiary register for `مشروع المؤاخاة`. Martyr (`martyr_name`, `martyr_national_id` **UNIQUE**, `martyr_date_of_birth` nullable, `martyrdom_date`, `children_count` unsigned), guardian (`guardian_name`, `guardian_national_id` nullable, `guardian_date_of_birth` nullable, `guardian_phone`), `account_holder_name`, `account_id` (**UNIQUE** FK→`accounts`, `restrictOnDelete`), `notes`, `created_by`/`updated_by`, timestamps, SoftDeletes. **There is no `age` column** — martyr age is computed in completed years **at the date of martyrdom**, never against today. Payment details (account number, bank type, currency, IBAN) are **never duplicated here**; they live only on `accounts`.
- `muwakha_family_projects` — first-class link (not a plain pivot: it carries `card_code`). `muwakha_family_id`, `project_id`, nullable `card_code`, timestamps, **no SoftDeletes**. `UNIQUE(muwakha_family_id, project_id)` + `UNIQUE(project_id, card_code)` — MySQL treats NULLs as distinct, so any number of families in one project may have no card code.
- `muwakha_family_accounts` (2026-08-17) — **durable family ↔ Account ownership**: `muwakha_family_id`, `account_id`, `account_holder_name`, timestamps, `UNIQUE(muwakha_family_id, account_id)`. Deliberately **no** `UNIQUE(family, currency)` (a family may hold several Accounts in one currency) and **no** duplicated payment field — `accounts` stays authoritative for currency, number, bank and IBAN. `account_holder_name` lives here because it is the one material identity field `accounts` has no column for. It is **ownership, not history**: no effective dates, versions, ordering or snapshots. `cascadeOnDelete` on the family is a hard-delete safety net only — the ordinary family soft delete keeps every mapping.
- **`muwakha_families.account_id` is the CURRENT destination; a family may own several Accounts over time**, including several in the same currency. `App\Services\Muwakha\MuwakhaFamilyService` owns every path; a **family delete leaves every Account and every mapping completely untouched** and only removes the project links.
- **Canonical Account name (2026-08-17):** `أسرة الشهيد {martyr_name} - {currency display name} - ({account_code})`, e.g. `أسرة الشهيد أحمد محمد - شيكل اسرائيلي - (123456)`. The rule lives once in `App\Support\Muwakha\MuwakhaAccountIdentity::accountNameFor()`; the currency label is `currencies.name` (`MuwakhaReference::currencyDisplayName()`), the same convention `AccountForm`/`AccountsTable` use. The bank name is deliberately excluded. The number is in the name so the **general Accounts screen** is self-describing — which matters because `account_code` is intentionally non-unique.
- **A Muwakha Account is IMMUTABLE — 2026-08-17.** `MuwakhaFamilyService` issues **no Account UPDATE anywhere**; the only Account write in the class is a CREATE. Historical `transaction_line` rows reference `accounts.id`, so rewriting an Account's identity would render today's payment details against yesterday's money. Material identity = canonical name + `account_type_id` (`أفراد`) + `currency_id` + `account_code` + `bank_type_id` + normalized `iban` + `account_holder_name`, built and compared by `MuwakhaAccountIdentity` (blank IBAN ≡ NULL; the **account number is not normalized** — OMS has no such rule and leading zeros are entered on purpose).
- **Edit resolution:** identity identical → only the family row is written; an **exact** match exists among the Accounts already mapped to **this** family → that Account is **REUSED** (repoint `account_id`, write no Account and no mapping); otherwise → exactly one new Account + one new mapping + the repoint. All atomic; any failure leaves the family on its previous Account. Non-account family data (guardian fields, children count, notes, project links, card codes) never moves the Account. **Reuse is strictly family-scoped through `muwakha_family_accounts` — never a global search and never a `LIKE` on name or number**, because two unrelated families may share a real bank number and ledgers are separated only by `accounts.id`. An exact match that is **inactive or soft-deleted fails safely** in Arabic rather than being revived or bypassed with a duplicate.
- **There is deliberately no account-history table and no account-change event system.** The superseded `accounts` rows — name, code, bank, IBAN, balance, active flag, currency and full ledger intact — are the history. Audit subject `muwakha_family_account` records a new mapping; a **reused** Account receives no event, because nothing wrote to it.
- **Eligibility:** OMS has **no project-to-project parent relation**. A Project is a Muwakha project exactly when `projects.project_super_id` = the `مشروع المؤاخاة` row in `projects_super` (shipped as `MUWAKHA_001`). `App\Support\Muwakha\MuwakhaReference` is the single definition, used by both the Select options and server-side validation, and resolves `أفراد` and the root by **name, never by id**, failing closed. The root is a `ProjectSuper`, so it can never appear in a project Select. The currency is **not** among these lookups — it is an operator choice, re-validated server-side against the same live-currency query the Select was built from and rejected as a field-level `ValidationException` on `currency_id`.
- **A deleted current Account is reported, not described.** When `muwakha_families.account_id` points at a soft-deleted Account, `بيانات الحساب` on the View page shows only `الحساب الحالي محذوف` and hides its name, currency, number, bank type, IBAN and account-holder. Display rule only — the mapping stays, `account_id` is not repointed, nothing is restored or reactivated.
- **Initial project links can be entered on the Family CREATE form** in the section `ربط بمشاريع المؤاخاة` (a create-only Repeater: `المشروع` from the eligible-project options + optional `رقم البطاقة`). Rows are replayed through `MuwakhaFamilyProjectService::link()` **inside** `MuwakhaFamilyService::create()`'s transaction, so eligibility/duplicate/card-code rules are the existing ones, duplicates *within one submission* are caught, and a rejected row rolls back the family, the Account, the ownership mapping and every earlier link. `update()` ignores the key; `ProjectsRelationManager` still owns post-create link management. No schema change — `project_id`/`card_code` are **not** on `muwakha_families`.
- **The Family VIEW page shows every mapped Account** in the read-only section `الحسابات المرتبطة بالأسرة` (الحالة / اسم الحساب / العملة / رقم الحساب / نوع البنك / IBAN / اسم صاحب الحساب), current Account badged `الحساب الحالي` and listed first, then previous Accounts newest-mapping-first (`MuwakhaFamily::visibleAccountLinks()`). The holder name is read from **each mapping**, so historical rows keep their own. **Soft-deleted Accounts are excluded by a display filter only** — mappings are never removed and no Account is restored or reactivated, so a deleted Account can never be badged current. No actions of any kind are exposed there; the account name links to the normal Accounts view page only when `AccountResource::canView()` passes.
- **The family register export shows the CURRENT Account only** (holder, number, bank, IBAN, currency) — one row per family, never one per historical Account. Historical financial reports stay correct on their own, because old Transactions still reference their immutable old Account ids.
- **There is no Muwakha payment engine, monthly-cycle table or scheduler.** A family's Account is simply selectable as the beneficiary in the existing صرف مبالغ التنفيذ workflow; Transaction posting, balances and FX are unchanged.
- **`accounts.account_code` is NOT unique** (see the `accounts` note above) — two families may share one real bank/wallet number.

**Family Account Statement — `كشف حساب الأسرة` (2026-08-18, reporting only):**
- **A per-family financial report, not a التقارير page.** `App\Filament\Resources\MuwakhaFamilies\Pages\MuwakhaFamilyAccountStatement` is a **resource page** at `admin/muwakha-families/{record}/account-statement`, registered as `MuwakhaFamilyResource::getPages()['account-statement']`. It registers **no navigation item** and has **no family selector**: the family is the report's subject, fixed by the route binding, and `$record` carries Livewire's `#[Locked]`. The one entry point is the header action `كشف حساب الأسرة` on the Family **View** page.
- **Scope = every Account in `muwakha_family_accounts`**, not `muwakha_families.account_id`. Current, previous, several in one currency, several currencies — all of them. Ownership is **never** inferred from an Account name or number (`account_code` is non-unique).
- **Soft-deleted mapped Accounts ARE included, and this is the deliberate opposite of the Family View page.** The two answer different questions: the View page asks *where may money be sent* (deleted Accounts hidden — operational safety), the statement asks *where has money already gone* (deleted Accounts included — historical completeness). `MuwakhaFamilyAccountStatementService` resolves Accounts with `withTrashed()` and never filters `accounts.deleted_at`; the widening is confined to that one service. Deleted Accounts appear in the Account filter labelled `{name} — محذوف` and are never restored, reactivated or written to.
- **Filters:** `من تاريخ` / `إلى تاريخ` (both **optional**; blank = no bound, both blank = the family's whole history; a reversed range is rejected on the page and again in the service), `الحساب` (default `كل الحسابات`, options = the mapped Accounts only, submitted value re-validated server-side against `muwakha_family_accounts` → 403 on a forged id), `نوع الحركة` (default `كل الحركات`, from `transactions.transaction_type_id`, options limited to the types that actually occur on the family's Accounts), `المشروع` (default `كل المشاريع`, see below). Nothing loads until `عرض`; any filter change clears the result, and both exports read that snapshot.
- **Project filtering uses TWO authoritative structured paths, and the second one is mandatory.** Path A is line level — `transaction_lines.project_cost_id → projects_costs.project_id` — which is what `ComprehensiveFinancialTransactionsReportService` uses. Path A **alone would be silently wrong here**: `CreateExecutionPayment::buildLines()`/`EditExecutionPayment` write their two lines with `project_cost_id = NULL`, and an Execution Payment is precisely how money reaches a Muwakha family Account. Path B therefore traces the business record — `transactions.id ← project_cost_budgets_payments.transaction_id → project_cost_budgets.project_cost_id → projects_costs.project_id` — the same chain `ProjectCostBudgetsPaymentObserver::resolveProjectId()` already relies on. Both are expressed as correlated `EXISTS` subqueries (never joins, which could multiply a ledger row and double a total). The two are mutually exclusive by construction, so there is no precedence question.
- **`muwakha_family_projects` plays NO part in Project filtering.** A family being linked to a Project says nothing about whether a given transaction on its Accounts belongs to that Project. The Project options are the Projects the family's own movements actually resolve to; a Project the family is merely linked to is not offered and is rejected server-side. Movements with no Project at all stay visible only under `كل المشاريع` — the existing Comprehensive-report convention, so there is no `بدون مشروع` option.
- **Grouped by CURRENCY, never by Account.** Sections come from `transaction_lines.currency_id` (the currency the amount is actually denominated in); several mapped Accounts sharing one currency **merge into one chronological section**, each row naming its own Account. Debit/credit totals accumulate inside a group only, which makes a blended `ILS + USD` figure structurally impossible rather than merely absent.
- **Columns (exactly seven):** `التاريخ` · `رقم المعاملة` · `نوع الحركة` · `البيان / الملاحظات` · `الحساب` · `مدين` · `دائن`. Debit/credit are `transaction_lines.debit_base`/`credit_base` **copied verbatim** — the same fields Trial Balance, the Comprehensive report, `JournalBalanceIntegrityChecker` and `FinancialTransactionBalanceGuard` read; no sign is invented and nothing is derived from a type name. The description follows the Comprehensive convention (`transactions.description` — `transaction_lines.notes`, `-` when both blank). Ordering is **newest first** (`t.transaction_time DESC, t.id DESC, tl.id DESC` — deterministic tie-break, never storage order).
- **No balances of ANY kind.** No opening, previous, running, cumulative or closing balance. The generic Account Statement's opening/running logic was deliberately **not** copied. Per currency section there are exactly two figures: `إجمالي المدين` and `إجمالي الدائن`.
- **One data builder, three consumers.** `App\Services\Muwakha\MuwakhaFamilyAccountStatementService::generate()` returns the normalized snapshot; the Blade page, `MuwakhaFamilyAccountStatementExcelExportService` (PhpSpreadsheet, RTL, A4 landscape, existing OMS palette) and `MuwakhaFamilyAccountStatementWordExportService` (PhpWord, default RTL, Tahoma 10, A4 portrait) all render **that array**. No accounting query is duplicated, and the exports cannot drift from the screen or exceed the applied filters. Account numbers and national ids are written as explicit strings so leading zeros survive.
- **Authorization reuses the existing matrix — no new permission.** View = `muwakha_families.view` on the record (enforced on mount and on every Livewire hydration); export additionally = `muwakha_families.export`, re-checked as the first statement of both export methods. So Super Admin / Admin / Project Manager view and export, **Viewer views but cannot export**, Accountant has no access.
- **Export audit** uses the existing `report_export` architecture: `ReportExportSubject::MuwakhaFamilyAccountStatement` (`muwakha_family_account_statement`, label `كشف حساب الأسرة`), one REQUIRED `export_requested` event per export, recorded only after authorization and after the `عرض` gate. The payload carries filters, ids and counts — never a row, and never `martyr_national_id` / `guardian_national_id` / `guardian_phone`, whose redaction for the `muwakha_family` CRUD subject must not be undone by an export event.
- **Empty state:** `لا توجد حركات مالية مطابقة للفلاتر المحددة.` The page still shows بيانات الأسرة, the filters and الحسابات المشمولة.
...
2. **صرف مبلغ المشروع** (Disbursement) — resource `ProjectCostBudgetsPaymentResource`, **model `ProjectCostBudget`**, slug `project-cost-budgets-disbursements`, prefix `BUD-YYYY-XXXX`. Saves to `project_cost_budgets` (`transaction_id` NOT NULL). Up to 4 accounts (source credit + **optional** admin debit + **optional** transfer debit + destination debit); 3 inputs: admin %, transfer %, fx_rate; manual "احسب" button. Source/admin/transfer = cost currency; destination = disbursement currency. **2–4 lines** tagged by notes-constants `LINE_SOURCE` / `LINE_ADMIN` / `LINE_TRANSFER` / `LINE_DESTINATION` — see **Optional deduction lines** below.
...
5. **التحويلات العامة** (General exchanges) — model `GeneralExchange`, slug `general-exchanges`, prefix `EXT-YYYY-XXXX`. Like disbursement (up to 4 accounts + 3 percentages + احسب) but with no project. **2–4 lines** via notes-tags, with the same optional-deduction rule — see **Optional deduction lines** below.
...
**Reference-data note (2026-08-16).** Until this date the repository had **no reference-data shipping pattern at all** — `DatabaseSeeder` seeds only permissions, settings and the bootstrap Super Admin, and every live `bank_types`/`currencies`/`accounts_type`/`projects_super` row had been entered by hand through the Filament UI. Migration `2026_08_16_100003_seed_muwakha_reference_data.php` establishes the pattern: an **idempotent data migration** matching on **name** (never a hard-coded id) that skips when a row of that name already exists **including a soft-deleted one**, so it can neither create a duplicate name nor resurrect a lookup an administrator deliberately deleted. Its `down()` is a deliberate no-op — the rows may already be referenced by real `accounts.bank_type_id`/`projects.project_super_id` values. It added four bank/payment types (`بنك الإسكان`, `بنك القدس`, `فودافون كاش`, `محفظة مؤقت`) and the `مشروع المؤاخاة` root ProjectSuper.
...
### Optional deduction lines — BUD / EXT (2026-08-19)

**`administrative_percentage` and `transfer_percentage` may legitimately be `0`, and a 0% deduction produces NO transaction line for that role.** This applies identically to صرف مبلغ المشروع (`BUD-`) and التحويلات العامة (`EXT-`).

A line carrying `debit_base = credit_base = amount_currency = 0` is meaningless accounting. `FinancialTransactionBalanceGuard::assertValidLinePayload()` rejects it and **must keep rejecting it** — the fix is never to build the line, never to weaken the guard.

| admin % | transfer % | lines | roles |
|---|---|---|---|
| > 0 | > 0 | **4** | source + admin + transfer + destination |
| = 0 | > 0 | **3** | source + transfer + destination |
| > 0 | = 0 | **3** | source + admin + destination |
| = 0 | = 0 | **2** | source + destination |

**Source and destination always exist** — there is no disbursement or exchange without money leaving one account and arriving in another. A 2-line BUD/EXT is still a **multi-currency** shape and may carry `fx_rate != 1`; it must never be treated as an ordinary single-currency pair.

Consequences, all enforced in code:

- **Line payloads are keyed by ROLE, not by array position.** `buildLines()` on all four Create/Edit pages returns `['source' => …, 'admin' => …?, 'transfer' => …?, 'destination' => …]`, and callers read `$lines['admin'] ?? null`. The old `$lines[0..3]` positional access would silently shift a destination line into an administrative slot at 2 or 3 lines.
- **`FinancialTransactionBalanceGuard::assertBalancedMultiCurrencyLines()`** takes `?array $adminLine` / `?array $transferLine`. A null line contributes **0** to `amount_after_deductions`; every FX, currency, direction and cent check is otherwise unchanged.
- **Account validation is conditional.** A 0% deduction's account is never handed to `FinancialAccountGuard` at all — not its existence, type, bank type, currency or active state. `FinancialAccountGuard` itself is unchanged; the callers simply omit the spec.
- **Balance mutations are conditional.** Source and destination always move; a deduction account moves only when that deduction exists. No zero-value increment is ever issued.
- **Audit roles are conditional.** A 0% deduction contributes no `<role>_account_id` to the snapshot. On an edit from positive to 0 the key is therefore *absent* from the new snapshot, which is what makes `FinancialAuditDiff` report the account as removed; the diff payload itself carries an explicit `null` on the new side with the old label preserved.
- **`JournalBalanceIntegrityChecker`** recognises all four multi-currency role sets (`MULTI_CURRENCY_ROLE_SETS`). `['destination','source']` is deliberately listed there and **not** in `SINGLE_CURRENCY_ROLE_SETS`.
- **`TransactionFlowClassifier`** (backfill) accepts a subset of the four notes tags, with `LINE_SOURCE` and `LINE_DESTINATION` required. Unknown tags, duplicate roles and missing required tags still throw.
- **A percentage > 0 whose derived amount rounds to 0.00 is REJECTED**, on its own percentage field, by `FinancialAmountGuard::assertDeductionsAreRecordable()` — e.g. 0.4% of 1.00. Such a deduction can neither be written (zero line forbidden) nor silently dropped (the operator asked for it). After that assertion, `percentage > 0` and `amount > 0` are equivalent, which is what keeps the line payload, account validation, balances and audit roles in agreement.
- **UI:** the four account cards are separate `Section`s in a `Grid::make(['default' => 1, 'md' => 2])` (2×2 desktop, stacked mobile). A deduction card stays **visible but disabled** at 0%, its `required()` rules are conditional, and its account cascade is cleared the moment the percentage reaches 0. Raising the percentage again re-enables an **empty** card — no previous selection is ever restored.
- **No historical migration was written.** Pre-existing 4-line BUD/EXT transactions containing zero-valued deduction lines are untouched; they still classify (all four tags present) but `JournalBalanceIntegrityChecker` reports them as `invalid_fx_base_conversion`, exactly as it did before this change.

**Error visibility (same change).** All three financial guards throw `ValidationException` keyed by DOMAIN field (`original_amount`, `admin_account_id`, and `lines` for every line-level check), while a Filament form resolves errors by STATE PATH (`data.original_amount`). Livewire stores the bag verbatim and Filament sends no notification, so every guard rejection used to render as a silent no-op. The four Create/Edit pages now use `App\Filament\Concerns\ReportsFinancialValidationFailures`, which shows a danger notification carrying the guard's own Arabic messages and then **re-throws unchanged** — rollback and validation semantics are untouched. The wider guard-key refactor (prefixing every throw with `data.`) is deliberately NOT done and remains open.
...
- Transaction line role tags (in `notes`): `LINE_SOURCE`, `LINE_ADMIN`, `LINE_TRANSFER`, `LINE_DESTINATION`, `LINE_BENEFICIARY`. These stay the authoritative tags that edit flows and reports match on — untouched by the 2026-07-14 `line_role` feature. Since 2026-08-19, `LINE_ADMIN` and `LINE_TRANSFER` are **optional** on BUD/EXT transactions (see **Optional deduction lines**): a consumer must treat their absence as a legitimate 0% deduction, never as missing data.

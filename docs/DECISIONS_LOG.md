...
### Date
2026-07-30 (Roadmap cleanup — the PDF/mPDF export phase is cancelled)

### Decision
**The "Excel + PDF (Arabic) export" roadmap item is CANCELLED, and no PDF or mPDF work is planned.** `mPDF` is removed from the tech stack as a planned technology. The accepted state is recorded instead: the reports are **working and approved**, their exports are delivered in **Excel (`xlsx`) and Word (`docx`) where implemented**, and **no report or export task is pending**. **No next system phase is set** — the remaining `OMS_Master_Reference.md` §6 bullets are open candidates only, none started or scheduled.

### Reason
The documentation had drifted into contradicting itself. §6 presented Excel + PDF Arabic export as the approved next phase and the tech-stack table listed "PDF (planned) | mPDF", while §2's audit contract — written later and verified against the code — records that `ReportExportFormat` has only `xlsx` and `docx` cases and that **no export service in this codebase produces CSV or PDF**. A search of `composer.json` confirms no PDF package is installed at all. Leaving the plan in place kept presenting finished, approved reports as if they still needed export development, which would mislead the next reader about what is actually outstanding.

The historical sentences were **annotated rather than deleted**, for the same reason the earlier receipt correction was: three separate log entries had recorded the export as the next phase at the time they were written, and rewriting them would erase a true record of what was believed. The roadmap bullet itself is struck through and labelled **CANCELLED** rather than removed, so anyone who remembers the approved plan can see explicitly that it was cancelled instead of finding it silently absent.

### Impact
The roadmap no longer claims any pending report or export work, and nothing in the six documents now presents PDF as planned. The two genuine non-code items are unaffected and remain the only outstanding acceptance work: the repo-wide Pint/formatting decision (351 of 712 files, no `pint.json`) and the real production restore drill. With no next phase set, the next piece of work requires a fresh explicit request rather than inheriting a stale roadmap entry. Should Arabic PDF export ever be wanted again, it re-enters as a new proposal — the previous approval is spent, and this entry is the record of that.
...
### Impact
The Audit initiative closes with no outstanding code defect. Three items leave this phase as explicitly non-code work: the pint/formatting decision, the production restore drill, and (from `OMS_Master_Reference.md` §6) confirmation of which pending roadmap item is the true next phase — the approved Excel/PDF Arabic export, or the double-entry receipt defect *then recorded as open*, which would be correctness work and arguably outrank it. **Superseded 2026-07-30:** the receipt defect was confirmed already resolved — the receipt flow posts a balanced debit/credit line pair and `oms:check-financial-integrity` reports no unbalanced transactions — so it was removed from the roadmap and the export stood as the next phase with no competing candidate. **Superseded again 2026-07-30 (see the entry above):** the Excel/PDF Arabic export item is itself **cancelled** — the reports are working and approved, exports ship as Excel and Word where implemented, and no PDF/mPDF work is planned — so the "which item is the true next phase" question is closed with **no report or export task pending**. The `event_action`-only index question is now a recorded, reasoned "not yet", so a future phase that observes a real slow filter on a large table can revisit it with evidence instead of re-deriving the argument. Anyone re-running this acceptance should expect the real-local audit count to keep growing whenever the application is used, and should capture a fresh baseline immediately before the run rather than relying on a figure from an earlier session.
